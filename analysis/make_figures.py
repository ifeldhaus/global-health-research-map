...
from constants import HIGH_INCOME_ISO2, WHO_REGIONS  # noqa: E402
...
# 16 distinct colours, one per journal, validated colourblind-safe on a light
# surface (passes lightness / chroma / CVD-separation / normal-vision gates) and
# ordered so confusable hues are never adjacent; white segment edges add relief.
SAFE = ['#2a78d6', '#eb6834', '#4a3aa7', '#008a00', '#00a5c4', '#e34948',
...
def fig3_externally_led():
    ...
    rows = con.execute(f"""
        WITH sc AS (
          ...
          FROM works w WHERE {SUB} AND w.study_country IS NOT NULL
            AND w.study_country NOT IN ('GLOBAL','UNKNOWN')
            AND w.study_country NOT LIKE '%|%'
            AND w.publication_year BETWEEN {Y0} AND {Y1})
        SELECT sc.yr, sc.sctry,
               MAX(CASE WHEN a.institution_country = sc.sctry THEN 1 ELSE 0 END) has_local
        FROM sc JOIN authorships a ON sc.id = a.openalex_id
        WHERE a.institution_country IS NOT NULL AND a.institution_country <> ''
        GROUP BY sc.id, sc.yr, sc.sctry""").fetchall()
    from collections import defaultdict
    ov = defaultdict(lambda: [0, 0])          # yr -> [externally-led, total]
    rg = defaultdict(lambda: [0, 0])          # (region, yr) -> [externally-led, total]
    for yr, sctry, has_local in rows:
        ext = 0 if has_local else 1
        ov[yr][1] += 1; ov[yr][0] += ext
        region = WHO_REGIONS.get(sctry)
        if region:
            rg[(region, yr)][1] += 1; rg[(region, yr)][0] += ext
    names = {'AFRO': 'Africa', 'AMRO': 'Americas', 'EMRO': 'E. Mediterranean',
             'EURO': 'Europe', 'SEARO': 'South-East Asia', 'WPRO': 'W. Pacific'}
    fig, ax = plt.subplots(figsize=(7.4, 4.4))
    for i, region in enumerate(sorted({k[0] for k in rg})):
        pts = [(yr, 100.0 * v[0] / v[1]) for (r, yr), v in sorted(rg.items())
               if r == region and v[1] >= 20]
        if len(pts) >= 3:
            ax.plot([p[0] for p in pts], [p[1] for p in pts], color=OKABE[i % len(OKABE)],
                    lw=1.2, alpha=0.7, marker='.', ms=4, label=names.get(region, region))
    ovpts = [(yr, 100.0 * v[0] / v[1]) for yr, v in sorted(ov.items()) if v[1] > 0]
    ax.plot([p[0] for p in ovpts], [p[1] for p in ovpts], color='black', lw=2.8,
            marker='o', ms=4, label='Overall', zorder=5)
